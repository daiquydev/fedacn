...
function anchorForCategory(category: string, userIndex: number): { lat: number; lng: number; label: string } {
    const c = category || ''
    // Add offset based on userIndex to spread them out
    const latOffset = (userIndex % 4 - 2) * 0.01
    const lngOffset = (Math.floor(userIndex / 4) - 2) * 0.01
...
async function run() {
    try {
        ...
        console.log(`Processing ${participants.length} participants for "${challenge.title}"`)
        ...
        for (let pIdx = 0; pIdx < participants.length; pIdx++) {
            ...
            console.log(`[${pIdx + 1}/${participants.length}] Processing user: ${email} (${userId})`)
            ...
            const anchor = anchorForCategory(challenge.category, pIdx)
            const goalKm = challenge.goal_value

            for (let dIdx = 0; dIdx < dayKeys.length; dIdx++) {
                const dayKey = dayKeys[dIdx]
                
                const km = goalKm + (dIdx % 3) * 0.1 + (pIdx % 5) * 0.05 // variety
                const hourVN = 5 + (dIdx % 4) + (pIdx % 3) // Spread out time
                const minuteVN = 10 + (dIdx * 7 + pIdx * 13) % 45
                const endTime = atVN(dayKey, hourVN, minuteVN)
                
                const avgKmh = 9 + (Math.random() * 3)
                const durationMin = Math.round((km / avgKmh) * 60)
                const startMs = endTime.getTime() - durationMin * 60 * 1000
                
                const seed = pIdx * 1000 + dIdx * 10 + 7
                const { points, lengthM } = await buildGpsRouteForSeed({
                    baseLat: anchor.lat + (Math.random() - 0.5) * 0.002,
                    baseLng: anchor.lng + (Math.random() - 0.5) * 0.002,
                    targetKm: km,
                    bearingDeg: (pIdx * 45 + dIdx * 20) % 360,
                ...
                const activity = await ActivityTrackingModel.create({
                    ...
                    name: `${challenge.category} - Session ${dIdx + 1}`,
                    status: 'completed',
                ...
                await ChallengeProgressModel.create({
                    ...
                    notes: `Ngày ${dIdx + 1}: Hoàn thành ${km.toFixed(2)}km ${challenge.category}`,
                    distance: km,
        ...
        console.log('Successfully generated data for all participants.')

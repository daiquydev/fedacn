...
async function seedOutdoorChallengesImproved() {
  await mongoose.connect(envConfig.mongoURL)
  console.log('Connected to MongoDB')

  // We can filter to start from the specific challenge ID or process all
  ...
  for (const challenge of challenges) {
    ...
    if (challenge.title.includes('trekking')) { centerLat = 16.1972; centerLng = 107.8597; }
    else if (challenge.category.includes('Đua xe')) { centerLat = 21.0583; centerLng = 105.8250; }
    else if (challenge.category.includes('Chạy bộ')) { centerLat = 10.7744; centerLng = 106.6924; }
    ...
    for (const participant of participants) {
      ...
      while (currentDate.isSameOrBefore(limitDate)) {
        ...
        // 80% chance to have some activity
        const hasActivityToday = Math.random() > 0.20
        
        if (hasActivityToday) {
          activeDays.add(dateStr)
          
          // Randomly decide if this user fails the goal today (15% chance of failing even if they ran)
          const willFailToday = Math.random() < 0.15
          ...
          for (let s = 0; s < numSessions; s++) {
            ...
            // If failing, keep distance very low. Otherwise, aim for target.
            let distM = 0
            if (willFailToday) {
               // Total day km will be less than challenge.goal_value
               distM = (challenge.goal_value * 0.2 * Math.random()) * 1000 / numSessions
            } else {
               // Total day km will be >= challenge.goal_value
               distM = (challenge.goal_value * (1.0 + Math.random() * 0.5)) * 1000 / numSessions
            }
            ...
            dayTotalKm += (distM / 1000)
            totalDistanceKm += (distM / 1000)
          }

          if (dayTotalKm >= challenge.goal_value) {
      ...
      // IMPORTANT: Update current_value based on goal_type
      // If goal_type is daily_km, current_value is the NUMBER OF DAYS completed.
      const finalValue = challenge.goal_type === 'daily_km' ? completedDays.size : totalDistanceKm
      ...
      await ChallengeParticipantModel.updateOne(
        { _id: participant._id },
        {
          $set: {
            ...
            status: finalValue >= participant.goal_value ? 'completed' : 'in_progress',
            is_completed: finalValue >= participant.goal_value
          }
        }
      )
    }
  ...
  console.log('ALL outdoor challenges re-seeded successfully with corrected logic!')
  await mongoose.disconnect()
}

seedOutdoorChallengesImproved().catch(console.error)

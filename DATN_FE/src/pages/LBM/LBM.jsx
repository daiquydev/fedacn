...
export default function LBM() {
  ...
  const onSubmit = handleSubmit((data) => {
    ...
    calculateLBMMutation.mutate(data, {
      onSuccess: (res) => {
        const lbmValue = res.data.result
        setDataLBM((prev) => ({ ...prev, LBM: lbmValue }))
        // Auto-save
        saveLBMMutation.mutate({ ...data, LBM: lbmValue }, {
          onSuccess: (saved) => {
            toast.success('Đã tính và lưu chỉ số LBM')
    ...
    <>
      <div className='grid xl:mx-4  pt-2 xl:gap-3 xl:grid-cols-6'>
        ...
        <div className='col-span-6 order-first xl:order-last my-3 xl:my-0 xl:col-span-2'>
          <CalculatorSidebar
            title='Tính toán LBM'
            subtitle='(Khối lượng cơ nạc)'
            gradient='from-violet-500 to-purple-400'
            result={(profile?.LBM || dataLBM.LBM) ? {
              value: profile?.LBM || dataLBM.LBM,
              unit: 'kg',
              label: 'Khối lượng cơ thể không tính mỡ'
            } : null}
            onAIClick={(profile?.LBM || dataLBM.LBM) ? () => setIsAIModalOpen(true) : null}
          >
            <form onSubmit={onSubmit} noValidate className='space-y-3'>
              ...
              <div>
                ...
                <div className='flex gap-4'>
                  <div className='flex items-center'>
                    <input type='radio' defaultChecked name='default-radio' value='male' {...register('gender')} id='male' className='radio radio-success' />
              ...
              <div className='pt-1'>
                {calculateLBMMutation.isPending ? (
                  <button disabled className='w-full py-2.5 rounded-xl bg-gradient-to-r from-violet-500 to-purple-400 text-white font-semibold flex items-center justify-center gap-2 opacity-70'>
                    <Loading classNameSpin='inline w-5 h-5 text-white/60 animate-spin fill-white' /> Đang tính...
                  </button>
                ) : (
                  <button className='w-full py-2.5 rounded-xl bg-gradient-to-r from-violet-500 to-purple-400 hover:from-violet-600 hover:to-purple-500 text-white font-semibold shadow-md hover:shadow-lg transition-all duration-200'>
                    Tính toán
                  </button>
                )}
              </div>
          ...
          <AIAnalysisModal
            ...
            calculationType='LBM'
            inputData={{ weight: dataLBM.weight, height: dataLBM.height, gender: dataLBM.gender }}
            calculatedResult={{ LBM_kg: profile?.LBM || dataLBM.LBM }}
          />
        </div>
      </div>
    </>

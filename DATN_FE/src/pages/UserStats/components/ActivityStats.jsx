    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="flex flex-col items-center bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
          ...
          <p className="text-xl font-semibold dark:text-white">{summary.totalTrainings}</p>
        </div>
        ...
        <div className="space-y-3">
          {processedActivities.map((activity) => (
            <div key={activity.id} className="flex items-start border-b border-gray-200 dark:border-gray-700 pb-3 last:border-0">
              <div className={`rounded-full w-8 h-8 flex items-center justify-center mr-3 mt-1 ${activity.type === 'event' ? 'bg-blue-100 dark:bg-blue-900' : 'bg-yellow-100 dark:bg-yellow-900'
                }`}>
                {activity.type === 'event' ? (
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${activity.status === 'completed' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' :

...
import { reportSportEvent } from '../../../apis/sportEventApi'
...
export default function ModalReportSportEvent({ onClose, eventId }) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const reportMutation = useSafeMutation({
    mutationFn: (body) => reportSportEvent(eventId, body)
  })

  const onSubmit = handleSubmit((data) => {
    reportMutation.mutate(data, {
      onSuccess: () => {
        toast.success('Báo cáo sự kiện thành công, quản trị viên sẽ xử lý')
        reset()
        onClose()
      },
      onError: (err) => {
  ...
  return (
    <ModalLayout closeModal={onClose} title='Báo cáo sự kiện' icon={FaFlag} size='md'>

            ...
            <div className='grid xl:grid-cols-3 items-center'>
              <div className='col-span-2 mb-2'>
                <div className='text-xl font-medium mb-2 flex items-center'>
                  ...
                  <span>Thực đơn mới nhất</span>
                </div>
                <div className='border-b-[3px] w-[20%] border-green-500'></div>
              </div>
              <Link
                to='/cooking/recipe'
                className='col-span-1 xl:flex xl:justify-end dark:text-gray-300 font-medium text-gray-600 hover:text-green-700 cursor-pointer transition-all duration-100'
              >
        ...
        <div className='mx-2 mt-5 mb-14'>
          <div className='grid xl:grid-cols-3 items-center'>
            <div className='col-span-2 mb-2'>
              ...
              <div className='border-b-[3px] mb-2 w-[20%] border-green-500'></div>
            </div>
            <Link
              to='/album'
              className='col-span-1 xl:flex xl:justify-end dark:text-gray-300 font-medium text-gray-600 hover:text-green-700 cursor-pointer transition-all duration-100'
          ...
          {isLoadingAlbum ? (
            ...
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4 pt-5'>
              {albumsData?.data?.result.albums.map((album) => {
                return (
                  <AlbumCard
                    ...
                    countClass='leading-relaxed text-sm'
                    titleClass='h-[3.75rem] transition-all duration-300 line-clamp-2 hover:text-green-500 cursor-pointer mt-2 text-base font-bold'
                    album_count_name='thực đơn'
                  />
                )
              })}
            </div>
            ...
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4 pt-5'>
              {blogData?.data?.result.blogs.map((blog) => (
                <BlogCard
                  key={blog._id}
                  blogItem={blog}
                  imgClass='w-full max-h-[12rem] object-cover rounded-t-xl scale-100 overflow-hidden'
                  dateClass='flex text-xs items-center gap-4 pt-2 pb-1'
                  titleClass=' font-bold transition-all cursor-pointer line-clamp-2 hover:text-green-500'
                  descriptionClass='leading-relaxed text-sm line-clamp-2 mt-2 mb-3'
                  linkClass='inline-block font-bold hover:text-green-500 transition-all duration-300 ease-in-out'
                />
              ))}
            </div>
          )}
...
const ItemUser = ({ user }) => {
  ...
  return (
    <div className='flex justify-between items-center'>
      <div className='flex items-center mb-4 mt-2 gap-3'>
        ...
        <div>
          ...
          >
            {user.name}
            {user.role === 1 && (
              <div className='text-green-500 rounded-full flex justify-center items-center'>
      ...
      <button onClick={handleFollow} className='btn btn-sm text-white hover:bg-green-700 bg-green-600'>
        Theo dõi
      </button>
    </div>
  )

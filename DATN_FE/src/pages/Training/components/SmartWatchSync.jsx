...
const SmartWatchSync = ({ Training, onActivityComplete }) => {
  ...
  const progressPercentage = Math.min(Math.round((totalCompleted / Training.targetValue) * 100), 100);
  ...
  const endActivity = () => {
    ...
    // Hiển thị kết quả
    toast.success(`Hoạt động hoàn thành! Bạn đã đạt được ${activityValue} ${Training.targetUnit}`);
  ...
  const dataType = getSmartWatchDataType(Training.category);
      ...
      <div className="flex mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        ...
        <div className="flex-1 text-center">
          ...
          <div className="text-2xl font-bold text-green-600 dark:text-green-400">
            {totalCompleted.toFixed(1)}/{Training.targetValue} {dataType.unit}
          </div>
        </div>
        <div className="flex-1 text-center">

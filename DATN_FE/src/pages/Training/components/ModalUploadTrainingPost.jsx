...
import { createPost } from '../../../apis/postApi' // Điều chỉnh thành API đăng bài đến Training
...
export default function ModalUploadTrainingPost({ closeModalPost, profile, Training, userProgress }) {
  ...
  const uploadMutation = useSafeMutation({
    mutationFn: (body) => {
      // Đây sẽ là API tạo bài đăng cho Training thay vì bài đăng thông thường
      // Giả sử API có thêm tham số TrainingId
  ...
  const handleUpload = async () => {
    ...
    if (includeProgress) {
      ...
      formData.append('targetValue', Training.targetValue)
      formData.append('targetUnit', Training.targetUnit)
      formData.append('progress', Training.progress)
    }
    ...
    uploadMutation.mutate(formData, {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: ['TrainingPosts', Training.id] })
  ...
  // Hiển thị icon của Training dựa vào category
        ...
        <div className='text-center'>
          ...
          <div className='bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 p-3 my-2 rounded-lg flex items-center'>
            ...
            <div className='text-left'>
              <h4 className='font-semibold text-gray-900 dark:text-gray-100'>{Training.title}</h4>
              <div className='text-xs text-gray-500 dark:text-gray-400 flex items-center'>
                <FaChartLine className='mr-1' /> 
                Tiến độ: {userProgress?.currentValue || 0}/{Training.targetValue} {Training.targetUnit} ({Training.progress}%)
              </div>
            </div>
          </div>
          ...
          <section className='w-full mx-auto items-center'>
            ...
            <textarea
              ...
              placeholder={`Chia sẻ trải nghiệm của bạn về thử thách ${Training.title}...`}
            ...
            <div className='flex items-center mb-3 bg-gray-50 dark:bg-gray-800 p-2 rounded-lg'>
              ...
              <label htmlFor="includeProgress" className="text-sm text-gray-700 dark:text-gray-300 cursor-pointer flex items-center">
                ...
                Hiển thị tiến độ hiện tại ({Training.progress}%) trong bài đăng
              </label>
            </div>

...
import { getLeaderboard } from '../../../apis/trainingApi'

export default function TrainingLeaderboard({ TrainingId }) {
  ...
  const { data: leaderboardData } = useQuery({
    queryKey: ['Training-leaderboard', TrainingId],
    queryFn: () => getLeaderboard({ TrainingId, page: 1, limit: 50 }),
    staleTime: 1000,
    enabled: !!TrainingId
  })

  const rawParticipants = leaderboardData?.data?.result?.leaderboard || leaderboardData?.data?.result || []
  const participants = rawParticipants.map((p, index) => ({
    id: p._id || p.user_id,
    name: p.name || p.user_name || 'Ẩn danh',
    avatar: p.avatar || '',
    progress: p.progress || 0,
    currentValue: p.current_value || p.total_checkins || 0,
    lastActive: p.last_active || p.last_checkin || '',
    streak: p.streak || p.current_streak || 0,
    rank: p.rank || index + 1
  }))
                ...
                <tr key={participant.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      {participant.rank <= 3 ? (
                        <FaMedal
                          className={`mr-2 ${participant.rank === 1 ? 'text-yellow-400' :

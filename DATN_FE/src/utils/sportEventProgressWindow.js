 ...
 */
export function isDailySportEventProgressAllowedAt(startDate, endDate, now = vnMoment()) {
  if (startDate == null || startDate === '') return false
  const t0 = vnMoment(startDate)
  if (!t0.isValid()) return false
  const endM = endDate != null && endDate !== '' ? vnMoment(endDate).endOf('day') : null
 ...
 */
export function nextDailySportEventProgressWindowOpensAt(startDate, endDate, now = vnMoment()) {
  if (startDate == null || startDate === '') return null
  const t0 = vnMoment(startDate)
...
export function isBeforeSportProgressWindow(startDate, now = vnMoment()) {

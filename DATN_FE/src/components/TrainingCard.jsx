...
/**
 * TrainingCard component for displaying community Trainings with participants
 * 
 * @param {Object} props
 * @param {Object} props.Training - The Training data to display
 * @param {Array} props.participants - Participants data with follow status
 * @param {Function} props.onJoin - Function to call when user joins the Training
 */
const TrainingCard = ({ Training, participants = [], onJoin }) => {
    ...
    >
      ...
      <div className="relative h-48">
        ...
        <div className="absolute top-3 left-3 bg-white dark:bg-gray-900 text-blue-500 font-medium px-3 py-1 rounded-full text-sm flex items-center">
          ...
          <span>{Training.category || 'Thử thách'}</span>
        </div>
      ...
      <div className="p-4">
        ...
        <div className="mb-4">
          <ParticipantsList
            ...
            initialLimit={4}
            size="sm"
            title={`${Training.participants || Training.participantCount || 0} người tham gia`}
            showCount={false}
          />
        </div>
        ...
        <div className="mt-auto pt-2 border-t border-gray-100 dark:border-gray-700">
          {Training.isJoined ? (
            ...
            <button
              onClick={(e) => e.stopPropagation()}
              className="w-full py-2 bg-gray-200 text-gray-500 rounded-md text-sm font-medium flex justify-center items-center cursor-default dark:bg-gray-700 dark:text-gray-400"
            >
              Đã kết thúc
            </button>

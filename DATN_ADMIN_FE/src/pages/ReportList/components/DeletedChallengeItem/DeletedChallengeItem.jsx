...
import { restoreChallenge } from '../../../../apis/inspectorApi'
import ConfirmBox from '../../../../components/GlobalComponents/ConfirmBox'
import { useSafeMutation } from '../../../../hooks/useSafeMutation'
import ChallengeInfoModal from '../ChallengeReportItem/ChallengeInfoModal'

export default function DeletedChallengeItem({ challenge: ch }) {
  const [openRestore, setOpenRestore] = useState(false)
  const [showInfoModal, setShowInfoModal] = useState(false)

  const restoreMutation = useSafeMutation({
    mutationFn: () => restoreChallenge(ch._id)
  })
  ...
  const handleRestore = () => {
    restoreMutation.mutate(null, {
      ...
      onError: () => toast.error('Không thể khôi phục thử thách')
    })
  }

  const creator = ch?.creator
  const reportCount = ch?.report_count ?? 0
  const deletedAt = ch?.deleted_at
    ...
    <>
      ...
      {openRestore && (
        <ConfirmBox
          ...
          subtitle='Thử thách sẽ hiển thị lại cho người dùng (trừ khi bị gỡ lần nữa).'

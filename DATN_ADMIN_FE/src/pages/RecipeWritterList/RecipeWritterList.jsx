...
import { getCategoryRecipes, getListRecipesForWritter } from '../../apis/writterApi'
...
export default function RecipeWritterList() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['recipes-writter-list', queryConfig],
    queryFn: () => {
        ...
        <div className=''>
          <div className='grid xl:grid-cols-6 '>
            ...
            <div className='col-span-4 lg:col-span-5 mb-2  '>
              <div className='flex flex-wrap gap-3 xl:justify-end items-center'>
                <button
                  onClick={() => navigate('/recipes-writter/create')}
          ...
          {data?.data.result.totalPage > 1 && (
            <div className='flex justify-center items-center'>
              <Pagination pageSize={data?.data.result.totalPage} queryConfig={queryConfig} url='/recipes-writter' />
            </div>
          )}
        </div>
      </div>

...
import {
    ...
    FaTimes, FaExclamationTriangle, FaSync
} from 'react-icons/fa'
...
function EventFormModal({ event, categories, onClose, onSuccess }) {
    ...
    const inputCls = (name) =>
        `w-full px-3 py-2 border rounded-lg dark:bg-slate-700 dark:text-white text-sm outline-none focus:ring-2 transition-all ${errors[name] ? 'border-red-400 focus:ring-red-300' : 'border-gray-300 dark:border-slate-600 focus:ring-blue-400 focus:border-blue-400'}`
    const labelCls = 'block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1'
    const sectionCls = 'bg-gray-50 dark:bg-slate-700/40 rounded-xl p-4 space-y-4'
    ...
    return (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4' onClick={(e) => e.target === e.currentTarget && onClose()}>
            <div className='bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col'>
                {/* Header */}
                <div className='flex items-center justify-between px-6 pt-5 pb-4 border-b border-gray-200 dark:border-slate-700 shrink-0'>
                    ...
                    <button onClick={onClose} className='p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg transition-colors'><FaTimes className='text-gray-500' /></button>
                </div>
                ...
                <form onSubmit={handleSubmit} className='overflow-y-auto flex-1 px-6 py-4 space-y-5'>
                    ...
                    <div>
                        ...
                        <div className={sectionCls}>
                            ...
                            <div>
                                ...
                                <input name='location' value={form.location} onChange={handleChange} className={inputCls('location')} placeholder='Tên địa điểm / URL...' />
                    ...
                    <div className='flex justify-end gap-3 pt-2 border-t border-gray-200 dark:border-slate-700'>
                        ...
                        <button type='submit' disabled={submitting}
                            className='px-5 py-2 text-sm bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center gap-2'>
...
export default function AdminSportEvent() {
    ...
    const { data, isLoading, isError, error } = useQuery({
        queryKey: ['adminSportEvents', page, search, filterCategory, filterEventType, filterStatus, sortBy],
                ...
                <div className='flex flex-wrap gap-3 items-end'>
                    ...
                    <form onSubmit={handleSearch} className='flex flex-1 min-w-[200px] gap-2'>
                        <div className='relative flex-1'>
                            ...
                            <input
                                ...
                                className='w-full pl-9 pr-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-blue-500'
                            />
                        </div>
                    ...
                    <div className='flex items-center gap-2'>
                        ...
                        <select
                            ...
                            className='px-3 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg dark:bg-slate-700 dark:text-white outline-none focus:ring-2 focus:ring-blue-500'
                        >
                            {EVENT_TYPE_OPTIONS.map(t => <option key={t} value={t}>{t === 'all' ? 'Tất cả loại' : t}</option>)}
                        </select>
                    </div>
                    ...
                    >
                        ...
                        <option value='earliest'>Sắp diễn ra</option>
                    </select>
                </div>
            </div>
                        ...
                        <div className='overflow-x-auto'>
                            <table className='w-full divide-y divide-gray-200 dark:divide-slate-700'>
                                <thead className='bg-gray-50 dark:bg-slate-900'>
                                    <tr>
                                        {['STT', 'Sự kiện', 'Danh mục / Loại', 'Thời gian', 'Địa điểm', 'Người tham gia', 'Trạng thái', 'Hành động'].map(h => (
                                ...
                                <tbody className='divide-y divide-gray-100 dark:divide-slate-700'>
                                    {events.length === 0 ? (
                                        ...
                                        <tr key={ev._id} className={`hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors ${ev.isDeleted ? 'opacity-60' : ''}`}>
                                            {/* STT */}
                                            ...
                                            <td className='px-4 py-3 whitespace-nowrap'>
                                                ...
                                                <div className='mt-1 w-20 bg-gray-200 dark:bg-slate-600 rounded-full h-1.5'>
                                                    <div
                    ...
                    {totalPage > 1 && (
                        <div className='flex items-center justify-center gap-2 mt-5'>
                            <button
                                ...
                                className='px-3 py-1.5 text-sm bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors'
                            >
                                ← Trước
                            </button>
                                ...
                                .map(p =>
                                    typeof p === 'number' ? (
                                        <button
                                            key={p}
                                            onClick={() => setPage(p)}
                                            className={`w-8 h-8 text-sm rounded-lg font-medium transition-colors ${p === page ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-700'}`}
                                        >
                                            {p}
                                        </button>
                                    ) : (
